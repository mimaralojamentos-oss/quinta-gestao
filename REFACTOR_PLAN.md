...
- [x] **Passo 16** — Alocação automática de pagamentos → unificar (item 8) — commit feito, em 2 fases (ver nota abaixo)
...
**Passo 16 — motor único de alocação automática de pagamentos, em duas fases.** Trabalho grande, feito em 6 commits com paragens obrigatórias entre fases (zona sensível — dinheiro real).

*Fase 1 (unificação):* comparadas linha a linha as 3 implementações (`lib/rentPaymentPlan.ts`, e as cópias próprias de `PaymentModal.tsx` e `TenantModal.tsx`). A divergência mais importante: o motor partilhado só sabia lidar com um mês de renda de cada vez, ao contrário das duas cópias, que já percorriam todos os meses em atraso. Reescrito para multi-mês, reaproveitando `getMonthlyRentStatus`/nova `getSingleMonthRentStatus` (`lib/rentShortfall.ts`). Também corrigidas divergências menores já identificadas: dívida manual passa a usar sempre `getDebtRemaining` (fecha a nota do Passo 10), dívidas manuais passam a ser sempre pagas da mais antiga para a mais recente, crédito de adiantamento existente é sempre descontado primeiro (com o valor visível na pré-visualização), e os movimentos de caixa passam a ser criados dentro do motor (antes cada sítio duplicava esta lógica — um deles, o `TenantModal.tsx`, usava por engano `source:'renda'` para pagamentos de luz). Migrados os 3 ecrãs, um commit por passo, com prova de equivalência com dados reais em cada um.

*Fase 2 (caução):* acrescentada a caução em falta como 2ª prioridade (Renda → Caução → Luz → Dívidas), reaproveitando `lib/depositShortfall.ts` sem duplicar a conta. O crédito de adiantamento não se aplica à caução (só à renda). Como os 3 ecrãs já usam o motor único desde a Fase 1, esta mudança entrou em todo o lado de uma vez, num só commit. Verificado com o caso real do H35 (Samuel João Batista): renda + caução cobertas por completo, excedente correto, e o destino "Só eletricidade" confirmado a não tocar na caução.

**Nota fora do plano — 2026-08-22: `getMonthlyRentStatus` passa a respeitar `leases.end_date`.** Antes, o cálculo de rendas em falta (`lib/rentShortfall.ts`) percorria os meses desde o início do contrato até ao mês atual sem olhar para a data de fim nem para o estado — um inquilino que saísse, por exemplo, a 31/08, continuava a aparecer com a renda de setembro em dívida até alguém marcar o contrato como "Terminado" manualmente, e nessa altura a dívida desaparecia de repente por completo (o contrato sai das listas de "ativos"), escondendo também dívidas reais que existissem.

Regra adotada, **sem pro-rata**: um mês conta se o dia 1 desse mês for `<= end_date`. Sair a 31/08 → agosto é o último mês devido; sair a 15/09 → setembro ainda conta por inteiro (não há divisão pelo número de dias do mês). Contratos sem `end_date` não são afetados — comportamento idêntico ao anterior, confirmado com dados reais.

Atualizados os 4 sítios que chamam `getMonthlyRentStatus` (`inquilinos/page.tsx`, `relatorios/page.tsx` — só este precisou de passar a pedir `end_date` à base de dados, os outros já usavam `select('*')` — `TenantModal.tsx`, e o motor `lib/rentPaymentPlan.ts`). No motor, o destino "Só renda" também passou a respeitar esta regra, para um pagamento nunca poder ser distribuído para um mês posterior ao fim do contrato.

**Caução em falta (`lib/depositShortfall.ts`) não recebeu o mesmo tratamento, de propósito**: a caução é um valor único devido pela duração toda do arrendamento, não uma renda mês a mês — continua devida enquanto o inquilino ocupar o espaço, mesmo com uma data de saída já marcada mas o contrato ainda `ativo`. Ao contrário da renda, não faz sentido "parar de a cobrar" antes da saída efetiva.
