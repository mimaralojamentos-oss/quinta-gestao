...
import { Droplet, Trash2, X, ChevronDown, ChevronRight, Settings, Save, Pencil, Search, Printer } from 'lucide-react'
...
/**
 * Fecho de contas de água na saída de um inquilino — espelha o equivalente
 * de eletricidade (app/eletricidade/espacos/page.tsx).
 */
...
export default function ContadoresEspacosAguaPage() {
  ...
  const pricePerM3 = config?.price_per_m3 ?? 1.5
  const vatRate = config?.vat_rate ?? 0
  const priceWithVat = pricePerM3 * (1 + vatRate)
  const minCharge = config?.min_charge ?? 0

  async function fetchConfig() {
    const { data } = await supabase.from('water_config').select('*').single()
    if (data) {
      setConfig(data)
      setConfigForm({
        price_per_m3: data.price_per_m3.toString(),
  ...
  async function saveConfig() {
    ...
    await supabase.from('water_config').update({
      price_per_m3: parseFloat(configForm.price_per_m3),
  ...
  async function fetchAll() {
    ...
    const { data: spacesData } = await supabase
      .from('spaces')
      .select('id, ref, type, has_own_water_meter, tenant_id, tenant:tenants(name)')
      .eq('has_own_water_meter', false)
      .order('ref')
  ...
  async function executeReset() {
    ...
    try {
      ...
      const m3 = anterior != null ? parseFloat((novoValor - anterior).toFixed(2)) : null

      const { error: readingError } = await supabase.from('water_readings').insert({
        space_id: space.id,
      ...
      const { error: accError } = await supabase.from('water_readings')
      ...
      await logAccess({
        action: 'editar',
        page: '/agua/espacos',
        details: `Fecho de contas de água do ${space.ref} (${resetModal.tenantName}): leitura ${novoValor}${perdoadas > 0 ? `, ${perdoadas} cobrança(s) perdoada(s)` : ''}`,
      })

      const totalPerdoado = pendingCharges.reduce((s, c) => s + (c.amount - (c.amount_paid ?? 0)), 0)
      alert(
        `✅ Fecho de contas concluído no ${space.ref}.\n\n` +
        `Leitura de saída: ${novoValor} m³\n` +
  ...
  function printWater(space: Space) {
...
<head>
  <meta charset="UTF-8">
  <title>Leituras de Água — ${space.ref}</title>
  <style>
...
<body>
  ...
  <div class="info-box">
    ...
    <div class="info-item"><label>Preço/m³ (c/ IVA)</label><span>${priceWithVat.toFixed(4)} €</span></div>
  </div>
  ...
  /**
   * Recalcula a cadeia de leituras de um espaço — espelha recalcularCadeia
   * de eletricidade.
   */
  ...
  async function saveEditReading() {
    ...
    const { error } = await supabase.from('water_readings').update({
    ...
    await logAccess({
      action: 'editar',
      page: '/agua/espacos',
      details: `Editou leitura de água (${formatDate(editForm.reading_date)} · ${newValue} m³)`,
    })
  ...
  async function cobrarAgoraPartilhado(reading: Reading) {
    ...
    const r = await cobrarLeituraPartilhada(supabase, SERVICO, {
      reading: { id: reading.id, reading_date: reading.reading_date, units: reading.m3_consumed },
    ...
    await supabase.from('water_readings').update({
  ...
  async function handleCobrarAgora() {
    ...
    const { data: newCharge, error: chargeError } = await supabase.from('water_charges').insert({
      ...
      units: editReadingModal.m3_consumed,
      amount: remaining,
      paid: remaining === 0,
    ...
    if (chargeError) {
      alert(`Erro ao criar a cobrança de água: ${chargeError.message}`)
      setSaving(false)
      return
    }
  ...
  async function saveReading(destino: 'cobrar' | 'acumular' | 'oferta') {
    ...
    const m3Consumed = prevValue != null ? parseFloat((newValue - prevValue).toFixed(2)) : null
    const accumulatedSoFar = getAccumulatedAmount(space.id)
    const amountCalc = m3Consumed != null ? parseFloat((m3Consumed * priceWithVat + accumulatedSoFar).toFixed(2)) : null
    const charged = destino === 'cobrar' && amountCalc != null && amountCalc >= minCharge
    const oferta = destino === 'oferta'

    const { data: inserted, error: readingError } = await supabase.from('water_readings').insert({
      space_id: space.id,
      reading_date: readingForm.reading_date,
      reading_value: newValue,
      previous_value: prevValue,
      m3_consumed: m3Consumed,
    ...
    if (grupo.length > 0) {
      ...
      if (charged && amountCalc) {
        const r = await cobrarLeituraPartilhada(supabase, SERVICO, {
          reading: { id: inserted.id, reading_date: readingForm.reading_date, units: m3Consumed },
      ...
      if (lease) {
        const { error: chargeError } = await supabase.from('water_charges').insert({
          lease_id: lease.id,
        ...
        if (chargeError) {
          ...
          await supabase.from('water_readings').update({ charged: true, accumulated: false }).eq('id', inserted.id)
        }
      }
    }
  ...
  async function confirmarOferta(reading: Reading, cobrancas: any[], motivo: string | null): Promise<boolean> {
    if (cobrancas.length === 1) {
      const { error } = await supabase.from('water_charges').delete().eq('id', cobrancas[0].id)
      ...
      const { error } = await supabase.from('water_charges').delete().in('id', cobrancas.map(c => c.id))
  ...
  async function deleteReading(reading: Reading) {
    ...
    if (reading.share_split) {
      const { data } = await supabase.from('water_charges').select('id, paid').eq('reading_id', reading.id)
      daDivisao = data ?? []
    ...
    const extra = n > 0
      ? ` Também vai${n > 1 ? 'o' : ''} ser apagada${n > 1 ? 's' : ''} ${n} cobrança${n > 1 ? 's' : ''} de água associada${n > 1 ? 's' : ''} (não paga${n > 1 ? 's' : ''}).`
      : ''

    if (!confirm(`Apagar esta leitura?${extra}`)) return

    if (n > 0) {
      const { error } = await supabase.from('water_charges').delete().in('id', unpaidCharges.map(c => c.id))
      if (error) { alert(`Não foi possível apagar as cobranças:\n\n${error.message}`); return }
    }

    const { error: delError } = await supabase.from('water_readings').delete().eq('id', reading.id)
    ...
    <AppLayout>
      <div className="p-4 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Contadores dos Espaços</h1>
        ...
        {showConfig && canEdit && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-xs font-medium text-blue-700 block mb-1">Preço da água (€/m³, sem IVA)</label>
                <input type="number" step="0.001" className="w-full border border-blue-200 rounded-lg px-3 py-2 text-sm bg-white"
                  value={configForm.price_per_m3}
                  onChange={e => setConfigForm(f => ({ ...f, price_per_m3: e.target.value }))} />
              </div>
              <div>
            ...
            <div className="mt-3 bg-white border border-blue-100 rounded-lg px-4 py-2 text-sm text-blue-800">
              Preço final com IVA: <strong>
                {formatCurrency(parseFloat(configForm.price_per_m3 || '0') * (1 + parseFloat(configForm.vat_rate || '0') / 100))}
              </strong> /m³
            </div>
                ...
                <div key={space.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors"
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${space.tenant_id ? 'bg-blue-100' : 'bg-gray-100'}`}>
                        <Droplet className={`w-4 h-4 ${space.tenant_id ? 'text-blue-600' : 'text-gray-400'}`} />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-right">
                        ...
                        {lastReading?.reading_value != null && (
                          <p className="text-xs text-gray-400">{lastReading.reading_value} m³</p>
                        )}
                      </div>
                      {canEdit && !titular && (
                        <button
                      ...
                      {spaceReadings.length > 0 && (
                        <button
                          onClick={e => { e.stopPropagation(); printWater(space) }}
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {spaceReadings.map(r => (
                              ...
                              <tr className="hover:bg-gray-50">
                                ...
                                <td className="py-2 text-sm">{r.m3_consumed != null ? `${Number(r.m3_consumed).toFixed(2)} m³` : '—'}</td>
            ...
            {!resetConfirm ? (
              <>
                <p className="text-sm text-gray-500 mb-4">
                  Fecha as contas de água do inquilino que sai e deixa o contador pronto para o próximo.
                </p>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-800">
                      {resetModal.lastReading
                        ? `${resetModal.lastReading.reading_value} m³ · ${formatDate(resetModal.lastReading.reading_date)}`
                        : 'sem leituras'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input className="input" type="number" step="0.001" placeholder="ex: 912.500"
                  ...
                  {resetForm.reading_value && resetModal.lastReading && (
                    <p className={`text-xs px-3 py-2 rounded-lg ${
                      ...
                      {parseFloat(resetForm.reading_value) < resetModal.lastReading.reading_value
                        ...
                        : `Consumo desde a última leitura: ${(parseFloat(resetForm.reading_value) - resetModal.lastReading.reading_value).toFixed(2)} m³ — não vai ser cobrado a ninguém.`}
                    </p>
                  )}
              ...
              <>
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
                  ...
                  <ul className="text-sm text-amber-800 space-y-1.5 list-disc list-inside">
                    <li>
                      Registar leitura de saída de <strong>{resetForm.reading_value} m³</strong> a {formatDate(resetForm.reading_date)}
                    </li>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            {readingModal.lastReading && (
              <div className="bg-gray-50 rounded-lg p-3 mb-4 text-sm">
                ...
                <p className="font-medium text-gray-800">{readingModal.lastReading.reading_value} m³ — {formatDate(readingModal.lastReading.reading_date)}</p>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" step="0.01" className="input" placeholder="ex: 250"
              ...
              {readingForm.reading_value && readingModal.lastReading && (
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
                  ...
                  {(() => {
                    const m3 = parseFloat(readingForm.reading_value) - readingModal.lastReading!.reading_value
                    const acc = getAccumulatedAmount(readingModal.space.id)
                    const total = parseFloat((m3 * priceWithVat + acc).toFixed(2))
                    return (
                      <div className="text-xs text-blue-700 space-y-0.5">
                        <p>{m3.toFixed(1)} m³ × {formatCurrency(priceWithVat)}/m³ = {formatCurrency(m3 * priceWithVat)}</p>
                        ...
                        {grupoDe(readingModal.space.id).length > 0 && total > 0 && (
                          <SharePreview total={total} units={parseFloat(m3.toFixed(2))} ownerId={readingModal.space.id}
            ...
            <div className="space-y-4">
              ...
              {editForm.reading_value && editReadingModal.previous_value != null && (
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
                  <p className="text-xs font-medium text-blue-700 mb-1">Pré-visualização:</p>
                  {(() => {
                    const m3 = parseFloat(editForm.reading_value) - editReadingModal.previous_value!
                    const total = parseFloat((m3 * priceWithVat).toFixed(2))
                    return (
                      <div className="text-xs text-blue-700 space-y-0.5">
                        <p>{m3.toFixed(1)} m³ × {formatCurrency(priceWithVat)}/m³ = {formatCurrency(m3 * priceWithVat)}</p>
                        <p className="font-semibold text-blue-800 text-sm mt-1">Total: {formatCurrency(total)}</p>
              ...
              {!editReadingModal.charged && (editReadingModal.amount_calculated ?? 0) > 0 && editPartilhado.length > 0 && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                  ...
                  <SharePreview total={editReadingModal.amount_calculated ?? 0} units={editReadingModal.m3_consumed}
