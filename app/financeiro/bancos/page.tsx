...
import {
  ...
  ArrowDownRight, ChevronLeft, Loader2, Link2, X, ArrowRight
} from 'lucide-react'
...
export default function BankDetailPage({ params }: { params: { id: string } }) {
  ...
  // Step 1: Parse file and detect header row
  async function handleFileSelect(file: File) {
    ...
    try {
      ...
      // Load saved mapping for this bank
      const savedMapping = bank?.column_mapping

      setMapping(savedMapping ?? {
        date: suggest(['data mov', 'data mov', 'date', 'data']),
        description: suggest(['descri', 'movimento', 'detalhe', 'detail']),
        amount: suggest(['valor', 'amount', 'montante', 'importância']),
        balance: suggest(['saldo', 'balance']),
        type: suggest(['tipo', 'type', 'débito/crédito', 'debito']),
      })

      setImportStep('mapping')
    } catch (err) {
      alert('Erro ao ler o ficheiro. Verifica se é um ficheiro Excel ou CSV válido.')
    }
    setImporting(false)
  }

  // Step 2: After mapping confirmed, show preview
  ...
  async function handleImport() {
    ...
    try {
      ...
      for (const row of dataRows) {
        ...
        // Parse amount - handle debit/credit type column
        let amount: number
        ...
        // If type column says Débito, make negative
            ...
            {importStep === 'mapping' && (
              <div>
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-4 text-sm text-blue-700">
                  Encontrámos <strong>{parsedHeaders.length} colunas</strong> no ficheiro. Confirma a associação de cada campo:
                </div>
                ...
                <div className="flex justify-between mt-6">
                  ...
                  <button className="btn-primary" onClick={handleMappingConfirm}>
            ...
            {importStep === 'preview' && (
              <div>
                      ...
                      {previewRows.map((row, i) => {
                        ...
                        return (
                          <tr key={i}>
                            ...
                            <td className="px-3 py-2 max-w-[200px] truncate">{getVal(mapping.description)}</td>
                ...
                <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-sm text-emerald-700 mb-4">
                  ✅ Linhas duplicadas serão automaticamente ignoradas. O mapeamento será guardado para importações futuras deste banco.
                </div>
